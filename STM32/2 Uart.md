...
## 第11课 [UART]基础知识
1. 一个串口只能连接一个外部串口设备。
2. 接线：TX发送（Transmit），RX接收（receive）。一端的TX接另一端的RX。
   ![alt text](<assets/2 Uart/image-4.png>)
3. 串口通信的数据帧。由起始位、数据位、停止位组成。
   1. 起始位：高电平下拉低电平保持一位，代表起始位。
   2. 数据位：8位或者9位，包含或者不包含1位校验位。校验分为奇校验或者偶校验。
   3. 停止位：上拉高电平保持一位，代表停止位。
   ![alt text](<assets/2 Uart/image.png>)
   ![alt text](<assets/2 Uart/image-1.png>)
4. 波特率：每秒钟传输位的数量。9600或者115200等。9600代表每秒传输9600位，每位传输需要1/9600秒，约为0.1毫秒。115200每位约需要8.7微妙。
...
//返回值为三种：HAL_OK,HAL_ERROR,HAL_BUSY
```
举例：
```c
...
4. 核心程序：
   ...
   ```
5. 调试：在Keil中，点击放大镜进入调试。先运行到curse line，然后单步运行，查看串口调试助手接收到的数据。
## 第13课 [UART]简单数据接收实验
### HAL库函数
串口接收函数HAL_UART_Receive()
```c
...
//接受一个字节，超时时间为一直等待
uint8_t dataRcvd;
hal_uart_receive(&uart1,dataRcvd,1,HAL_MAX_DELAY);

//接收10个字节，超时时间为1000ms
uint8_t buffer[10];
hal_uart_receive(&uart1,buffer,sizeof(buffer),1000);

```
### 简单数据接收实验:串口接收到0就灭灯，接收到1就亮灯。
1. MX串口设置。
      1. Connectivity中，Usart1设置为Asynchronous模式。
      2. Usart1的Parameter Settings设置为：115200，8，None，1。
2. 接线。
   ![alt text](<assets/2 Uart/image-7.png>)
3. 串口调试助手设置。
   1. 设置115200，8，N，1。
   2. 打开串口
4. STM32核心代码：
